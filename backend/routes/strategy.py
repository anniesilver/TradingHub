...
from models.product import UserProduct, Product, PerformanceRecord
from app import db
...
def get_performance_history(user_product_id):
    ...
    return jsonify({
        ...
        "performance": round((user_product.current_balance - user_product.start_balance) / 
                            user_product.start_balance * 100, 2),
...
def run_strategy(user_product_id):
    ...
    if not start_date:
        latest_record = PerformanceRecord.query.filter_by(
            user_product_id=user_product_id
