...
class Product(db.Model):
    ...
    strategy_type = db.Column(
        db.String(50), nullable=False
    )  # e.g., "SPY_POWER_CASHFLOW"
...
class UserProduct(db.Model):
    ...
    performance_records = db.relationship(
        'PerformanceRecord', back_populates='user_product'
    )
...
class PerformanceRecord(db.Model):
    ...
    user_product_id = db.Column(
        db.Integer, db.ForeignKey('user_products.id'), nullable=False
    )
    ...
    user_product = db.relationship(
        'UserProduct', back_populates='performance_records'
    )
